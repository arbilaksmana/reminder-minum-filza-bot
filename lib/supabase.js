...
export async function upsertUser(tgId, name = 'User') {
...
export async function getPendingEvent(tgId) {
  const { data } = await supabase
    .from('events')
    .select('*')
    .eq('tg_id', String(tgId))
...
// Hash operations for duplicate detection
export async function isHashExists(sha256) {
  const { data } = await supabase
    .from('photo_hashes')
    .select('id')
    .eq('sha256', sha256)
...
export async function saveHash(sha256, driveFileId) {
  await supabase
    .from('photo_hashes')
